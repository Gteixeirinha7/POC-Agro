...
export default class OrderScreenCheckoutBarter extends LightningElement {
	...
	handleUploadFinished(event) {
		...
		this.dispatchEvent(new CustomEvent('uploadfinished', {detail: file}));
	}
	...
	onClickShowPedidoStep2aByCarScreenExcludeBarter(event){
		...
		{ icon: "warning", buttons: true }).then((action) => {
			if (action) {
				this.dispatchEvent(new CustomEvent('deleteproductconcludebarter', {detail: value}));
	...
	applyCampaignDiscounts(value){
		...
		this.orderObject.OrderItem.forEach(orderItem => {
				...
				orderItem.Campaign.forEach(campaign => {
					...
					var listPriceWithInterestAndDiscounts = parseFloat(orderItem.PriceList) + parseFloat(orderItem.AdditionValue) - parseFloat(orderItem.DiscountValue);
					...
					if ((!campaignProduct || campaignProduct.length == 0) || (campaignProduct && campaignProduct.CanReceiveDiscount)) {
						...
						if (!item.Campaign.find(cp => cp.id == campaign.CampaignId)) {
							...
							item.Campaign.forEach(campaign => {
								...
								var listPriceWithInterestAndDiscounts = parseFloat(item.PriceList) + parseFloat(item.AdditionValue) - parseFloat(item.DiscountValue);
